...
class Arquivo implements \Cnab\Retorno\IArquivo
{
	...
	public function __construct($codigo_banco, $filename)
	{
		...
		if(count($linhas) < 2)
			$linhas = explode("\n", $this->content);
		$this->header  = new \Cnab\Cnab240\HeaderArquivo($this->codigo_banco);
        $this->trailer = new \Cnab\Cnab240\TrailerArquivo($this->codigo_banco);
		...
		foreach($linhas as $linha)
        {
			...
			else if($tipo_registro == '1')
            {
                ...
                $lastLote->header = new \Cnab\Cnab240\HeaderLote($this->codigo_banco);
            ...
            else if($tipo_registro == '5')
            {
                ...
                $lastLote->trailer = new \Cnab\Cnab240\TrailerLote($this->codigo_banco);
